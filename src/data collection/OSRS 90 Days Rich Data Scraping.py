...
#OSRS Rich Dataset Code 

# Features
# 6H timeseries with volume for a maximum of 90 days data per item if available. 
...
# Config
# --------------------
# Original made as excel output but hit file limit due triple the timepoints per item, changed to full parquet only output.
...
OUTPUT_DIR = PROJECT_ROOT / "data" / "rich"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

PARTIAL_PATH = OUTPUT_DIR / "osrs_rich_timeseries_partial.parquet"
CHECKPOINT_PATH = OUTPUT_DIR / "rich_checkpoint.txt"
...
# Timeseries URL for selecting full series of 6H timepoint in order to aquire 90 day of data w/volume. 
MAPPING_URL    = "https://prices.runescape.wiki/api/v1/osrs/mapping"
TIMESERIES_URL = "https://prices.runescape.wiki/api/v1/osrs/timeseries"  # params: id, timestep=6h
...
def get_json_with_retries(url, params=None, max_retries=4, base_sleep=3):
    ...
    while True:
        try:
            ...
            #Retry if empty body (200 error but no data)
            if not resp.content:
                attempt += 1
                if attempt > max_retries:
...
def fetch_mapping():
    ...
    return items
# This function does the actual fetching of the 6 hour timepoint series (90 days) for a single item ID, then retries the entire wrapper. Gives warning if wrapper fully fails/no data output.
# Includes arguement if no legitimate data in 6H timepoint (untradable items, newly implemented item (missing price history) etc) 
def fetch_rich_timeseries(item_id):
    """Fetch 6H timeseries (with volume) for an item."""
    params = {"id": item_id, "timestep": "6h"}
    data = get_json_with_retries(TIMESERIES_URL, params=params, max_retries=4, base_sleep=3)
    if data is None:
        print(f"  -> Final: request failed for item {item_id}. Skipping.")
        return []
    arr = data.get("data", [])
    if not arr:
        print(f"  -> Final: no 6H data available for item {item_id}. Skipping.")
        return []
    print(f"  -> Success: received {len(arr)} 6H points for item {item_id}.")
    return arr
...
for idx, item in enumerate(all_items, start=1):
    ...
    print(f"[{idx}/{len(all_items)}] Fetching 6H rich timeseries for item: {name} (ID: {item_id})")
    series = fetch_rich_timeseries(item_id)

# Countdown output for cooldown
    requests_since_cooldown += 1
    if requests_since_cooldown >= COOLDOWN_EVERY_N_REQUESTS:
        print(f"== Cooldown: reached {COOLDOWN_EVERY_N_REQUESTS} timeseries requests. Sleeping {COOLDOWN_SECONDS}s ==")
    ...
    if series:
        for point in series:
            ...
            all_rows.append({
                ...
                "Avg High Price": point.get("avgHighPrice"),
                "Avg Low Price": point.get("avgLowPrice"),
                "High Price Volume": point.get("highPriceVolume"),
                "Low Price Volume": point.get("lowPriceVolume"),
            })
...
# --------------------
# Final Save + 24H Build
# --------------------

today_str = datetime.today().strftime("%Y-%m-%d")

rich_6h_df = pd.DataFrame(all_rows)

# Save raw 6H parquet
rich_6h_path = OUTPUT_DIR / f"osrs_rich_timeseries_6H_{today_str}.parquet"
rich_6h_df.to_parquet(rich_6h_path, index=False)
print(f"6H rich dataset saved as '{rich_6h_path}'")


# =========================
# Build 24H Daily Dataset
# =========================

rich_6h_df["Average Price 6H"] = (
    rich_6h_df["Avg High Price"] + rich_6h_df["Avg Low Price"]
) / 2

rich_6h_df["Total Volume 6H"] = (
    rich_6h_df["High Price Volume"].fillna(0)
    + rich_6h_df["Low Price Volume"].fillna(0)
)

rich_6h_df["Date"] = rich_6h_df["Timestamp"].dt.floor("D")

rich_daily = (
    rich_6h_df.groupby(["Item ID", "Item Name", "Date"], as_index=False)
    .agg({
        "Avg High Price": "mean",
        "Avg Low Price": "mean",
        "High Price Volume": "sum",
        "Low Price Volume": "sum"
    })
)

rich_daily["Average Price"] = (
    rich_daily["Avg High Price"] + rich_daily["Avg Low Price"]
) / 2

rich_daily["Total Volume"] = (
    rich_daily["High Price Volume"].fillna(0)
    + rich_daily["Low Price Volume"].fillna(0)
)

rich_daily = rich_daily[~rich_daily["Average Price"].isna()]

# Save 24H parquet
rich_daily_path = OUTPUT_DIR / f"osrs_rich_timeseries_24H_{today_str}.parquet"
rich_daily.to_parquet(rich_daily_path, index=False)

print(f"24H rich dataset saved as '{rich_daily_path}'")
